...
namespace ZAPConnect
{
    public class ModbusHandler
    {
        ...
        /// </summary>
        public static void ModbusSerialRtuMasterWriteRegisters(string portString, byte slaveId, ushort startAddress, ushort data)
        {
            using (SerialPort port = new SerialPort(portString))
            {
                ...
                ushort[] registers = new ushort[] { data };
        ...
        public static ushort[] ModbusSerialRtuMasterReadRegisters(string portString, byte slaveId, ushort startAddress, ushort points)
        {
            using (SerialPort port = new SerialPort(portString))
            {
                // configure serial port
                ...
                var values = master.ReadHoldingRegisters(slaveId, startAddress, points);
        ...
        /// </summary>
        public static ushort ModbusTcpMasterReadInputs()
        {
            using (TcpClient client = new TcpClient("127.0.0.1", 502))
            {
                ...
                for (int i = 0; i < numInputs; i++)
                    result = result + ("Input {0}={1}", startAddress + i, inputs[i] ? 1 : 0);
                ushort resultParsed = Convert.ToUInt16(result);
                return resultParsed;
            }
